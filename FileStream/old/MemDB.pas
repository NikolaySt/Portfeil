...
uses Forms, DbConsts {$IFDEF RX_D5}, ComObj {$ENDIF}, IniFiles, DBFileConst,
  ProtectData;
...
begin
  ...
  try
    if FDBMem.LoadFromFile(FTableName) then
    Load;
  finally
    FDBMem.Clear;
  end;
...
begin
  ...
  try
    if FDBMem.LoadFromStream(Stream) then
    Load;
  finally
    FDBMem.Clear;
  end;
