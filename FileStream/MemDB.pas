...
{                                                       }
{         Copyright (c) 2002 Nikolay Stoychev           }
...
uses Windows, SysUtils, Classes, Controls, DB, DBUtils, Variants, MemSave;
...
type
  ...
  TBaseMemoryData = class(TDataSet)
  private
    FRecordPos: Integer;
    FRecordSize: Integer;
...
{ TMemoryRecord }
  ...
  private
    FMemoryData: TBaseMemoryData;
    FID: Integer;
    FData: Pointer;
    FBlobs: Pointer;
    function GetIndex: Integer;
    procedure SetMemoryData(Value: TBaseMemoryData; UpdateParent: Boolean);
  protected
    procedure SetIndex(Value: Integer); virtual;
  public
    constructor Create(MemoryData: TBaseMemoryData); virtual;
    constructor CreateEx(MemoryData: TBaseMemoryData; UpdateParent: Boolean); virtual;
    destructor Destroy; override;
    property MemoryData: TBaseMemoryData read FMemoryData;
    property ID: Integer read FID write FID;
    property Index: Integer read GetIndex write SetIndex;
    property Data: Pointer read FData;
  end;

  TFieldOperation = (foSUM, foMIN, foMAX);

  TDataOperation = class(TCollectionItem)
  private
    FName: string;
    FFieldOperation: TFieldOperation;
    function GetFieldOperation: TFieldOperation;
    procedure SetFieldOperation(const Value: TFieldOperation);
  protected
  ...
  published
    ...
    property FieldOperation: TFieldOperation read GetFieldOperation write SetFieldOperation;
  end;


  TDataOperations = class(TOwnedCollection)
  private
    FDataSet: TDataSet;
    FUpdated: Boolean;
...
{ TCustomMasterTable }
  ...
  TDBLocalInfo = packed record
    FileCode: Word;         //упознавателен код
    Ver: Word;              //версия на файла
    Date: TDateTime;        //кога е създаден

    DataLength: Longint;    //за дължина на данните
    FileName: string[50];   //как се какзва
    Owner: string[50];      //кои е собственика на файла, т.е. кои го е създал
    LocalInfo: string[100]; // информация за копютъра на който е създаден
    Password: string[30];   //парола;
    OtherInfo: string[255]; //друга информация
  end;
  ...
  TCustomMasterTable = class(TBaseMemoryData)
  private
    FTableName: string;
    FDBMem: TDBMemory;
    FDBLocalInfo: TDBLocalInfo;
    FCompress: Boolean;
    FCoding: Boolean;
    FOnPassword: TOnPassword;
    FOperations: TDataOperations;
    function GetFieldType(FieldType: TFieldType): Longint; overload;
    function GetFieldType(FieldType: Longint): TFieldType; overload;
    procedure InternalPassword(var APassword: string; var ACancel: Boolean);
  protected
    procedure SetActive(Value: Boolean); override;

    procedure SaveDBLocalInfo; virtual;
    procedure SaveFields; virtual;
    procedure SaveRecords; virtual;

    procedure LoadDBLocalInfo; virtual;
    ...
    property Operations: TDataOperations read FOperations write FOperations;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    ...
    property DBLocalInfo: TDBLocalInfo read FDBLocalInfo write FDBLocalInfo;
  end;

  TMasterTable = class(TCustomMasterTable)
  published
    property Active;
    property Compress;
    property Coding;
  ...
  TDBMemSession = class(TComponent)
  private
    FPassList: TStringList;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure AddPassword(APassword: string);
    property Passwords: TStringList read FPassList;
  end;

var
  DBMemSession: TDBMemSession;  
...
uses Forms, DbConsts {$IFDEF RX_D5}, ComObj {$ENDIF}, IniFiles, ConstMemDB,
  ProtectData;
...
procedure Register;
begin
  RegisterComponents('BDE', [TMasterTable]);
end;
...
constructor TMemoryRecord.Create(MemoryData: TBaseMemoryData);
begin
  CreateEx(MemoryData, True);
end;

constructor TMemoryRecord.CreateEx(MemoryData: TBaseMemoryData;
...
procedure TMemoryRecord.SetMemoryData(Value: TBaseMemoryData; UpdateParent: Boolean);
var
  I: Integer;
  DataSize: Integer;
begin
...
constructor TBaseMemoryData.Create(AOwner: TComponent);
begin
...
destructor TBaseMemoryData.Destroy;
begin
  inherited Destroy;
  FreeIndexList;
  ClearRecords;
...
function TBaseMemoryData.GetCapacity: Integer;
begin
  if FRecords <> nil then Result := FRecords.Capacity
  else Result := 0;
end;

procedure TBaseMemoryData.SetCapacity(Value: Integer);
begin
  if FRecords <> nil then FRecords.Capacity := Value;
end;

function TBaseMemoryData.AddRecord: TMemoryRecord;
begin
  Result := TMemoryRecord.Create(Self);
end;

function TBaseMemoryData.FindRecordID(ID: Integer): TMemoryRecord;
var
  I: Integer;
begin
...
function TBaseMemoryData.InsertRecord(Index: Integer): TMemoryRecord;
begin
  Result := AddRecord;
  Result.Index := Index;
end;

function TBaseMemoryData.GetMemoryRecord(Index: Integer): TMemoryRecord;
begin
...
function TBaseMemoryData.BCDToCurr(BCD: Pointer; var Curr: Currency): Boolean;
begin
  Move(BCD^, Curr, SizeOf(Currency));
  Result := True;
end;

function TBaseMemoryData.CurrToBCD(const Curr: Currency; BCD: Pointer; Precision,
...
procedure TBaseMemoryData.InitFieldDefsFromFields;
var
  I: Integer;
  Offset: Word;
begin
...
function TBaseMemoryData.FindFieldData(Buffer: Pointer; Field: TField): Pointer;
var
  Index: Integer;
begin
{$IFDEF RX_D4}
...
function TBaseMemoryData.CalcRecordSize: Integer;
var
  I: Integer;
begin
  Result := 0;
...
procedure TBaseMemoryData.InitBufferPointers(GetProps: Boolean);
begin
...
procedure TBaseMemoryData.ClearRecords;
begin
...
function TBaseMemoryData.AllocRecordBuffer: PChar;
begin
...
procedure TBaseMemoryData.FreeRecordBuffer(var Buffer: PChar);
begin
  if BlobFieldCount > 0 then
...
procedure TBaseMemoryData.ClearCalcFields(Buffer: PChar);
begin
  FillChar(Buffer[FRecordSize], CalcFieldsSize, 0);
end;

procedure TBaseMemoryData.InternalInitRecord(Buffer: PChar);
var
  I: Integer;
begin
...
procedure TBaseMemoryData.InitRecord(Buffer: PChar);
begin
...
function TBaseMemoryData.GetCurrentRecord(Buffer: PChar): Boolean;
begin
  Result := False;
...
procedure TBaseMemoryData.RecordToBuffer(Rec: TMemoryRecord; Buffer: PChar);
var
  I: Integer;
begin
...
function TBaseMemoryData.GetRecord(Buffer: PChar; GetMode: TGetMode;
...
function TBaseMemoryData.GetRecordSize: Word;
begin
  Result := FRecordSize;
end;

function TBaseMemoryData.GetActiveRecBuf(var RecBuf: PChar): Boolean;
begin
  case State of
    dsBrowse:
...
function TBaseMemoryData.GetFieldData(Field: TField; Buffer: Pointer): Boolean;
var
  RecBuf, Data: PChar;
{$IFDEF RX_D5}
  VarData: Variant;
...
procedure TBaseMemoryData.SetFieldData(Field: TField; Buffer: Pointer);
var
  RecBuf, Data: PChar;
{$IFDEF RX_D5}
  VarData: Variant;
...
procedure TBaseMemoryData.SetFiltered(Value: Boolean);
begin
...
procedure TBaseMemoryData.SetOnFilterRecord(const Value: TFilterRecordEvent);
begin
  if Active then begin
    CheckBrowseMode;
...
function TBaseMemoryData.RecordFilter: Boolean;
var
...
function TBaseMemoryData.GetBlobData(Field: TField; Buffer: PChar): TMemBlobData;
begin
  Result := PMemBlobArray(Buffer + FBlobOfs)[Field.Offset];
end;

procedure TBaseMemoryData.SetBlobData(Field: TField; Buffer: PChar;
  Value: TMemBlobData);
begin
...
procedure TBaseMemoryData.CloseBlob(Field: TField);
begin
...
function TBaseMemoryData.CreateBlobStream(Field: TField; Mode: TBlobStreamMode): TStream;
begin
...
function TBaseMemoryData.BookmarkValid(Bookmark: TBookmark): Boolean;
begin
  Result := FActive and (TBookmarkData(Bookmark^) > Low(Integer)) and
    (TBookmarkData(Bookmark^) <= FLastID);
end;

function TBaseMemoryData.CompareBookmarks(Bookmark1, Bookmark2: TBookmark): Integer;
begin
...
procedure TBaseMemoryData.GetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  Move(PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData, Data^,
    SizeOf(TBookmarkData));
end;

procedure TBaseMemoryData.SetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  Move(Data^, PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData,
    SizeOf(TBookmarkData));
end;

function TBaseMemoryData.GetBookmarkFlag(Buffer: PChar): TBookmarkFlag;
begin
  Result := PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkFlag;
end;

procedure TBaseMemoryData.SetBookmarkFlag(Buffer: PChar; Value: TBookmarkFlag);
begin
  PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkFlag := Value;
end;

procedure TBaseMemoryData.InternalGotoBookmark(Bookmark: TBookmark);
var
  Rec: TMemoryRecord;
  SavePos: Integer;
  Accept: Boolean;
...
procedure TBaseMemoryData.InternalSetToRecord(Buffer: PChar);
begin
  InternalGotoBookmark(@PMemBookmarkInfo(Buffer + FBookmarkOfs)^.BookmarkData);
end;

procedure TBaseMemoryData.InternalFirst;
begin
  FRecordPos := -1;
end;

procedure TBaseMemoryData.InternalLast;
begin
...
procedure TBaseMemoryData.AssignMemoryRecord(Rec: TMemoryRecord; Buffer: PChar);
var
  I: Integer;
begin
// TODO : SaveRecord
...
procedure TBaseMemoryData.SetMemoryRecordData(Buffer: PChar; Pos: Integer);
var
  Rec: TMemoryRecord;
begin
  if State = dsFilter then Error(SNotEditing);
...
procedure TBaseMemoryData.SetAutoIncFields(Buffer: PChar);
var
  I, Count: Integer;
  Data: PChar;
begin
...
procedure TBaseMemoryData.InternalAddRecord(Buffer: Pointer; Append: Boolean);
var
  RecPos: Integer;
  Rec: TMemoryRecord;
begin
...
procedure TBaseMemoryData.InternalDelete;
var
  Accept: Boolean;
begin
...
procedure TBaseMemoryData.InternalPost;
var
  RecPos: Integer;
begin
...
procedure TBaseMemoryData.OpenCursor(InfoQuery: Boolean);
begin
...
procedure TBaseMemoryData.InternalOpen;
begin
...
procedure TBaseMemoryData.InternalClose;
begin
  ClearRecords;
  FAutoInc := 1;
  BindFields(False);
...
procedure TBaseMemoryData.InternalHandleException;
begin
  Application.HandleException(Self);
end;

procedure TBaseMemoryData.InternalInitFieldDefs;
begin
end;

function TBaseMemoryData.IsCursorOpen: Boolean;
begin
  Result := FActive;
end;
...
function TBaseMemoryData.GetRecordCount: Integer;
begin
  Result := FRecords.Count;
end;

function TBaseMemoryData.GetRecNo: Integer;
begin
  CheckActive;
  UpdateCursorPos;
...
procedure TBaseMemoryData.SetRecNo(Value: Integer);
begin
...
function TBaseMemoryData.IsSequenced: Boolean;
begin
  Result := not Filtered;
end;

function TBaseMemoryData.Locate(const KeyFields: string;
...
procedure TBaseMemoryData.EmptyTable;
begin
  if Active then begin
    CheckBrowseMode;
    ClearRecords;
...
procedure TBaseMemoryData.CopyStructure(Source: TDataSet);
...
function TBaseMemoryData.LoadFromDataSet(Source: TDataSet; RecordCount: Integer;
  Mode: TLoadMode): Integer;
var
...
function TBaseMemoryData.SaveToDataSet(Dest: TDataSet; RecordCount: Integer): Integer;
var
  MovedCount: Integer;
begin
  Result := 0;
...
procedure TBaseMemoryData.SortOnFields(const FieldNames: string;
{$IFDEF RX_D4}
  CaseInsensitive: Boolean = True; Descending: Boolean = False);
{$ELSE}
  CaseInsensitive, Descending: Boolean);
...
procedure TBaseMemoryData.Sort;
var
  Pos: TBookmarkStr;
begin
...
procedure TBaseMemoryData.QuickSort(L, R: Integer; Compare: TCompareRecords);
var
  I, J: Integer;
  P: TMemoryRecord;
begin
...
function TBaseMemoryData.CompareRecords(Item1, Item2: TMemoryRecord): Integer;
var
  Data1, Data2: PChar;
  F: TField;
  I: Integer;
...
function TBaseMemoryData.GetIsIndexField(Field: TField): Boolean;
begin
...
procedure TBaseMemoryData.CreateIndexList(const FieldNames: string);
var
  Pos: Integer;
  F: TField;
begin
...
procedure TBaseMemoryData.FreeIndexList;
begin
  FIndexList.Free;
  FIndexList := nil;
end;
...
begin
  FMode := Mode;
  FField := Field;
  FDataSet := FField.DataSet as TBaseMemoryData;
...
constructor TCustomMasterTable.Create(AOwner: TComponent);
begin
  inherited;
  FOperations := TDataOperations.Create(Self, Self, TDataOperation);
//  inherited Create(ADataSet, AOwner, TFieldDef);

  FDBMem := TDBMemory.Create(Self);
  FDBMem.OnPassword := InternalPassword;

  FCompress := False;
  Coding := False;

  FillChar(FDBLocalInfo, SizeOf(TDBLocalInfo), 0);
  with FDBLocalInfo do
  begin
    FileCode := file_code_begin;
    Ver := file_ver;
  end;
end;

destructor TCustomMasterTable.Destroy;
begin
  FDBMem.Free;
  FOperations.Free;
  inherited;
...
procedure TCustomMasterTable.InternalPassword(var APassword: string; var ACancel: Boolean);
begin
  if Assigned(FOnPassword) then
    FOnPassword(APassword, ACancel);
end;

function TCustomMasterTable.GetFieldType(FieldType: TFieldType): Longint;
begin
  case FieldType of
    ftString:      Result := file_ftString    ;
    ftSmallint:    Result := file_ftSmallint  ;
    ftInteger:     Result := file_ftInteger   ;
    ftWord:        Result := file_ftWord      ;
    ftBoolean:     Result := file_ftBoolean   ;
    ftFloat:       Result := file_ftFloat     ;
    ftCurrency:    Result := file_ftCurrency  ;
    ftBCD:         Result := file_ftBCD       ;
    ftDate:        Result := file_ftDate      ;
    ftTime:        Result := file_ftTime      ;
    ftDateTime:    Result := file_ftDateTime  ;
    ftBytes:       Result := file_ftBytes     ;
    ftVarBytes:    Result := file_ftVarBytes  ;
    ftAutoInc:     Result := file_ftAutoInc   ;
    ftADT:         Result := file_ftADT       ;
    ftFixedChar:   Result := file_ftFixedChar ;
    ftWideString:  Result := file_ftWideString;
    ftLargeint:    Result := file_ftLargeint  ;
    ftVariant:     Result := file_ftVariant   ;
    ftGuid:        Result := file_ftGuid      ;
    
    //BlobTypes
    ftBlob:        Result := file_ftBlob       ;
    ftMemo:        Result := file_ftMemo       ;
    ftGraphic:     Result := file_ftGraphic    ;
    ftFmtMemo:     Result := file_ftFmtMemo    ;
    ftParadoxOle:  Result := file_ftParadoxOle ;
    ftDBaseOle:    Result := file_ftDBaseOle   ;
    ftTypedBinary: Result := file_ftTypedBinary;
    ftOraBlob:     Result := file_ftOraBlob    ;
    ftOraClob:     Result := file_ftOraClob    ;
  else
    Result := file_ftInteger;
  end;
end;

function TCustomMasterTable.GetFieldType(FieldType: Longint): TFieldType;
begin
  case FieldType of
    file_ftString     :Result := ftString;
    file_ftSmallint   :Result := ftSmallint;
    file_ftInteger    :Result := ftInteger;
    file_ftWord       :Result := ftWord;
    file_ftBoolean    :Result := ftBoolean;
    file_ftFloat      :Result := ftFloat;
    file_ftCurrency   :Result := ftCurrency;
    file_ftBCD        :Result := ftBCD;
    file_ftDate       :Result := ftDate;
    file_ftTime       :Result := ftTime;
    file_ftDateTime   :Result := ftDateTime;
    file_ftBytes      :Result := ftBytes;
    file_ftVarBytes   :Result := ftVarBytes;
    file_ftAutoInc    :Result := ftAutoInc;
    file_ftADT        :Result := ftADT;
    file_ftFixedChar  :Result := ftFixedChar;
    file_ftWideString :Result := ftWideString;
    file_ftLargeint   :Result := ftLargeint;
    file_ftVariant    :Result := ftVariant;
    file_ftGuid       :Result := ftGuid;

      //BlobTypes
    file_ftBlob       :Result := ftBlob;
    file_ftMemo       :Result := ftMemo;
    file_ftGraphic    :Result := ftGraphic;
    file_ftFmtMemo    :Result := ftFmtMemo;
    file_ftParadoxOle :Result := ftParadoxOle;
    file_ftDBaseOle   :Result := ftDBaseOle;
    file_ftTypedBinary:Result := ftTypedBinary;
    file_ftOraBlob    :Result := ftOraBlob;
    file_ftOraClob    :Result := ftOraClob;
  else
    Result := ftInteger;      
  end
end;

procedure TCustomMasterTable.SetActive(Value: Boolean);
begin
  if not Active and (Value <> Active) and (FTableName <> '') then
    LoadFromFile;
...
procedure TCustomMasterTable.SaveDBLocalInfo;
begin


  FDBMem.PasswordMask :=
    GenerationKeyMask(FDBLocalInfo.Password, Length(FDBLocalInfo.Password));
    
  if FDBMem.PasswordMask <> 0 then FCoding := True;

  FDBMem.Compress := FCompress;
  FDBMem.Coding := FCoding;

  FDBMem.Write(FDBLocalInfo, SizeOf(FDBLocalInfo));
end;

procedure TCustomMasterTable.SaveFields;
var
  i: Integer;
  arr_field: array of TFieldInfo;
begin
  SetLength(arr_field, FieldDefs.Count);
  for i := 0 to FieldDefs.Count - 1 do
    with arr_field[i] do
    begin
      Name := FieldDefs[i].Name;
      DataType := GetFieldType(FieldDefs[i].DataType);
      Precision := FieldDefs[i].Precision;
      Size := FieldDefs[i].Size;
      Other := '';
    end;
  I := FieldDefs.Count;
  FDBMem.Write(I, SizeOf(Integer));
  FDBMem.Write(arr_field[0], SizeOf(TFieldInfo)*FieldDefs.Count);
end;

procedure TCustomMasterTable.SaveRecords;
var
  i, f, Size, Size_Blob: Integer;
  MemRec: TMemoryRecord;
  P: Pointer;
  Blob: TMemBlobData;
  next_blob: Integer;
begin
  FDBMem.Write(FRecords.Count, SizeOf(Integer));

  for i := 0 to FRecords.Count - 1 do
  begin
    MemRec := GetMemoryRecord(i);
    P := MemRec.FData;
    ...
    begin
      if FieldDefs[f].DataType in ftBlobTypes then
      begin
        Blob := PMemBlobArray(MemRec.FBlobs)[next_blob];
        Size_Blob := Length(Blob);

        FDBMem.Write(Size_Blob, SizeOf(Integer));
        FDBMem.Write(string(Blob)[1]{PChar(Blob)[0]}, Size_Blob);

        Inc(next_blob);
      end
      else begin
        Size := CalcFieldLen(FieldDefs[F].DataType, FieldDefs[F].Size);
        Size := Size + 1;

        FDBMem.Write(Size, SizeOf(Integer));
        FDBMem.Write(P^, Size);

        P := Pointer(Integer(P) + Size);
      end;
    end;
  end;
end;

procedure TCustomMasterTable.LoadDBLocalInfo;
begin
  FDBMem.Read(FDBLocalInfo, SizeOf(TDBLocalInfo));
  FCompress := FDBMem.Compress;
  FCoding := FDBMem.Coding;
end;

procedure TCustomMasterTable.LoadFields;
var
  i: Integer;
  arr_field: array of TFieldInfo;
  ...
  var
    index: Integer;
  begin
    index := FieldDefs.IndexOf(arr_field[i].Name);
    if index >= 0 then Result := True
    else Result := False;
  end;

begin
  FDBMem.Read(i, SizeOf(Integer));
  if i > 0 then SetLength(arr_field, i);

  FDBMem.Read(arr_field[0], SizeOf(TFieldInfo)*i);
  ...
  for i := 0 to i - 1 do
    if not IsField then
      ...
      begin
        ...
        DataType := GetFieldType(arr_field[i].DataType);
        Size := arr_field[i].Size;
        Precision := arr_field[i].Precision;
     //   CreateField(Self);
      end;
end;

procedure TCustomMasterTable.LoadRecords;
var
  i, count_blob, next_blob: Integer;
  count_rec: Integer;
  ...
  Rec: TMemoryRecord;
  MemDataRec: TMemoryStream;

  procedure GetMemForBlob;
  begin
    if count_blob > 0 then
    begin
      GetMem(Rec.FBlobs, count_blob * SizeOf(Pointer));
      Initialize(PMemBlobArray(Rec.FBlobs)[0], count_blob);
    end;
  end;

  procedure ReadCountBlob;
  var
    i: Integer;
  begin
    count_blob := 0;
  ...
  procedure ReadBlobFields;
  var
    Temp: TMemBlobData;
    B: array of byte;
  begin
    size_len := size_len;
    SetLength(B, size_len);

    FDBMem.Read(B[0], size_len);

    SetLength(Temp, size_len);

    Move(B[0], PChar(Temp)[0], size_len);
    PMemBlobArray(Rec.FBlobs)[next_blob] := string(Temp);
    Inc(next_blob);
  end;

  procedure ReadOtherFields;
  begin
    GetMem(Buffer, size_len);
    try
      FDBMem.Read(Buffer^, size_len);
      MemDataRec.Write(Buffer^, size_len);
    finally
      FreeMem(Buffer, size_len);
    end;
  end;

begin
  ReadCountBlob;
  MemDataRec := TMemoryStream.Create;
  try
    FDBMem.Read(count_rec, SizeOf(Integer));

    for i := 0 to count_rec - 1 do
    begin
      next_blob := 0;
      Rec := InsertRecord(i);
      GetMemForBlob;
      ...
      begin
        FDBMem.Read(size_len, SizeOf(Integer));
        if FieldDefs[n].DataType in ftBlobTypes then
          ReadBlobFields
        else
          ReadOtherFields;
      end;

      Move(MemDataRec.Memory^, Rec.FData^, MemDataRec.Size);
      MemDataRec.Clear;
    end;
  finally
    MemDataRec.Free;
...
procedure TCustomMasterTable.Load;
begin
  FDBMem.Position := 0;
...
procedure TCustomMasterTable.Save;
begin
  FDBMem.Position := 0;
...
procedure TCustomMasterTable.SaveToFile;
begin
  try
    Save;
    FDBMem.SaveToFile(FTableName);
  finally
    FDBMem.Clear;
  end;
end;

procedure TCustomMasterTable.SaveToStream(Stream: TStream);
begin
  FDBMem.Clear;
  try
    Save;
    FDBMem.SaveToStream(Stream);
  finally
    FDBMem.Clear;
  end;
end;

procedure TCustomMasterTable.LoadFromFile;
begin
  if Active then raise Exception.Create('Table is active');
  try
    if FDBMem.LoadFromFile(FTableName) then Load;
  finally
    FDBMem.Clear;
  end;
end;

procedure TCustomMasterTable.LoadFromStream(Stream: TStream);
begin
  if Active then raise Exception.Create('Table is active');

  try
    if FDBMem.LoadFromStream(Stream) then Load;
  finally
    FDBMem.Clear;
  end;
end;
...
procedure TDBMemSession.AddPassword(APassword: string);
var
  i: Integer;
begin
  i := FPassList.IndexOf(APassword);
  if i = -1 then
    FPassList.Add(APassword)
  else
    if FPassList.Strings[i] <> APassword then
      FPassList.Add(APassword);
end;

constructor TDBMemSession.Create(AOwner: TComponent);
begin
  inherited;
  FPassList := TStringList.Create;
end;

destructor TDBMemSession.Destroy;
begin
  FPassList.Free;
  inherited;
end;
...
constructor TDataOperations.Create(ADataSet: TDataSet; AOwner: TPersistent;
  AClass: TCollectionItemClass);
begin
...
procedure TDataOperations.DoUpdate(Sender: TObject);
begin
...
procedure TDataOperations.SetItemName(AItem: TCollectionItem);
begin
...
procedure TDataOperations.Update(AItem: TCollectionItem);
begin
...
constructor TDataOperation.Create(Collection: TCollection);
begin
  inherited;
end;

function TDataOperation.GetDisplayName: string;
begin
  Result := FName;
end;

function TDataOperation.GetFieldOperation: TFieldOperation;
begin
  Result := FFieldOperation;
end;

procedure TDataOperation.SetDisplayName(const Value: string);
begin
  {
  if (Value <> '') and (AnsiCompareText(Value, FName) <> 0) and
    (Collection is TDataOperations){ and
    //TDataOperations(Collection).IndexOf(Value) >= 0)
    then
...
procedure TDataOperation.SetFieldOperation(const Value: TFieldOperation);
begin
  if Value <> FFieldOperation then
    FFieldOperation := Value;
end;

initialization
  DBMemSession := TDBMemSession.Create(nil);
